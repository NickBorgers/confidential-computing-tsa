...
use cms::{CmsConfig, CvmComponents, build_signed_data};
use config::WrapperConfig;
use response::{build_timestamp_resp_success, build_timestamp_resp_rejection};
use rfc3161::{TimeStampReq, HashAlgorithm, RejectReason, build_cvm_request, validate_request};
use vsock_client::{CvmConfig, send_request, parse_response_status, parse_response_payload};
...
fn parse_timestamp_req_minimal(der: &[u8]) -> Result<TimeStampReq, RejectReason> {
    ...
    let (_, outer_header) = read_der_length(&der[1..])
        .ok_or(RejectReason::BadDataFormat)?;
    ...
    let (mi_len, mi_header) = read_der_length(&after_version[1..])
        .ok_or(RejectReason::BadDataFormat)?;
    ...
    let (algo_len, algo_header) = read_der_length(&mi_content[1..])
        .ok_or(RejectReason::BadDataFormat)?;
    ...
    let hash_algorithm = HashAlgorithm::from_oid(oid_bytes)
        .ok_or(RejectReason::BadAlg)?;
    ...
    let (digest_len, digest_header) = read_der_length(&mi_content[digest_start + 1..])
        .ok_or(RejectReason::BadDataFormat)?;
